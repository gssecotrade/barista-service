const coffees = {
    catuai: {
        handle: "catuai",
        name: "Catuai",
        reason: "Encaja por su perfil suave, equilibrado y versátil, ideal para empezar o para disfrutarlo con facilidad en distintos momentos.",
        image: "https://arte-coffee.com/cdn/shop/files/Catuai_Lavado.jpg?v=1747402022",
    ...
    geisha: {
        handle: "geisha",
        name: "Geisha",
        reason: "Encaja por su perfil floral, elegante y aromático, ideal para momentos más pausados y experiencias más delicadas.",
        image: "https://arte-coffee.com/cdn/shop/files/Geisha_Lavado.jpg?v=1747402022",
    ...
    pacamara: {
        handle: "pacamara",
        name: "Pacamara",
        reason: "Encaja por su mayor cuerpo, intensidad y complejidad, ideal para una experiencia más gastronómica o con más presencia en taza.",
        image: "https://arte-coffee.com/cdn/shop/files/Pacamara_Natural.jpg?v=1747402022",
...
function findCoffeeByName(name) {
    ...
    if (normalized.includes("catuai"))
        return coffees.catuai;
    if (normalized.includes("geisha"))
        return coffees.geisha;
    if (normalized.includes("pacamara"))
        return coffees.pacamara;
    return null;
}
export function detectIntent(message) {
    const msg = message.toLowerCase();
    if (msg.includes("estado de mi pedido") ||
...
export function recommendCoffee(message) {
    const s = extractSignals(message);
    if (s.floral)
        return coffees.geisha;
    if (s.intense || s.dessert)
        return coffees.pacamara;
    if (s.soft)
        return coffees.catuai;
    return null;
}
export function buildResponse(message, context) {
...
- Si quieres, en el siguiente paso te doy la receta exacta según V60, Chemex o el método que uses en casa.`,
    ...
    if (intent === "order_help") {
        return {
            intent,
            reply: "Puedo ayudarte con la compra directa. Si prefieres, dime qué perfil buscas y te orientaré hacia el café que mejor encaje. También puedo aclararte qué opción de suscripción puede interesarte más.",
        };
    }
    if (intent === "order_status") {
        return {
            intent,
            reply: "Puedo ayudarte a revisar si ya has realizado un pedido o el estado en que se encuentra. Para ello necesito el número de pedido o el correo con el que se hizo la compra.",
        };
    }
    if (intent === "complaint") {
        return {
            intent,
            reply: "Entiendo. Si has tenido una incidencia con un pedido, una entrega o un producto, indícame qué ha ocurrido y te ayudaré a encauzarlo de la forma más adecuada.",
        };
    }
    if (intent === "subscription_help") {
        return {
            intent,
            reply: "Puedo ayudarte a entender mejor la suscripción: qué ventajas ofrece, para quién encaja mejor y cómo sacar más valor según tu consumo.",
        };
    }
    if (intent === "professional") {
        return {
            intent,
            reply: "Si lo estás planteando desde una perspectiva profesional, puedo orientarte en selección de cafés, propuesta para carta, signature drinks, formación y experiencia de cliente.",
        };
    }
    if (intent === "education") {
        return {
            intent,
            reply: "Puedo ayudarte a profundizar en perfiles de taza, diferencias entre cafés, orígenes y formas de disfrutar mejor cada café según el momento y la preparación.",
        };
    }
    return {
